...
func insert() {
	...
	fmt.Scanln(&name)
	insrt, err := db.Query("INSERT INTO student(student_id, name) VALUES (?,?)", id, name)
	if err != nil {
...
func update() {
	...
	updt, err := db.Query("UPDATE employee SET name=? WHERE student_id=?", name, id)
	if err != nil {
		panic(err.Error())
	}
...
func delete() {
	...
	del, err := db.Query("DELETE FROM employee WHERE student_id=?", id)
	if err != nil {
		panic(err.Error())
	}
...
func findAll() {
	...
	var id int
	var name string
	find, err := db.Query("Select * from student")
	if err != nil {
		panic(err.Error())
	}
	for find.Next() {
